...
import "./index.css";
import { Radio, Search, Calendar, X, Play, Sun, Moon, List, Download } from "lucide-react";
import { format, parseISO } from "date-fns";

const API_URL = "/api/search";
const DROPS_API_URL = "/api/drops";
...
function App() {
	...
	useEffect(() => {
		...
		// Clear results if no query (for all modes)
		if (!query.trim()) {
			setResults([]);
			setIndexStatus("");
			setHasMore(false);
		...
		searchTimeout.current = setTimeout(async () => {
			...
			try {
				...
				if (showDrops) {
					...
					res = await axios.get(DROPS_API_URL, {
						params,
						signal: controller.signal,
					});
					// Mark each drop with type for rendering
					const dropsWithType = res.data.map((drop) => ({ ...drop, type: "drop" }));
					setResults(dropsWithType);
					setHasMore(false); // No pagination for drops
				} else {
					const params = { q: query };
					if (showBestOf) {
						params.type = "best_of";
				...
				if (totalFiles === -1) {
					axios
						.get("/api/status")
						.then((r) => setTotalFiles(r.data.totalFiles))
						.catch(() => {});
				}
			}
		}, 400); // 400ms delay
	...
	}, [query, selectedYears, retryTick, showBestOf, showDrops]);

	// Fetch all shows for the "Full List" tab
	useEffect(() => {
		if (activeTab === "shows") {
			const fetchShows = async () => {
				...
				try {
					if (showDrops) {
						// Fetch all drops
						const res = await axios.get(DROPS_API_URL, { params: { q: "" } });
						const dropsWithType = res.data.map((drop) => ({ ...drop, type: "drop" }));
						setAllShows(dropsWithType);
						setIndexStatus("");
					} else {
						const params = {};
						if (showBestOf) {
	...
	}, [activeTab, selectedYears, showBestOf, showDrops]);
	...
	const highlightText = (text, highlight, youtubeUrl) => {
		...
		return lines.map((line, lineIdx) => {
			const tsMatch =
				line.match(/^\[(\d{2}:\d{2}:\d{2}[.\d]*) -->/) || line.match(/^(\d{1,2}:\d{2}(?::\d{2})?)/);
			let jumpLink = null;
			...
			return (
				<div key={lineIdx} className="transcript-line">
					{jumpLink}
					{parts.map((part, i) =>
						expandedTerms.has(part.toLowerCase()) ? (
			...
			<div className="sticky-header">
				<header style={{ position: "relative" }}>
					...
					<h1 className="retro-title">
						<Radio
							style={{ display: "inline", marginRight: "10px", verticalAlign: "middle" }}
							size={32}
						/>
					...
					<div className="timeline">
						<div
							className={`timeline-chip ${
								selectedYears.length === 0 && !showBestOf && !showDrops ? "active" : ""
							}`}
							onClick={() => {
								setSelectedYears([]);
								setShowBestOf(false);
						...
						<div
							className={`timeline-chip ${showDrops ? "active" : ""}`}
							onClick={() => {
				...
				<div className="tabs">
					...
					>
						<List size={18} style={{ marginRight: "8px" }} /> FULL LIST
					</button>
				</div>
				...
				{activeTab === "search" && !loading && !indexStatus && query && (
					...
					>
						{showDrops
							? `FOUND ${results.length} DROPS`
							: `FOUND ${results.length}${hasMore ? "+" : ""} SEGMENTS IN ${(
									searchTime / 1000
							  ).toFixed(2)}s`}
					</div>
				)}
				...
				{activeTab === "shows" && !loadingShows && (
					...
					>
						LISTING {allShows.length} {showDrops ? "DROPS" : "SHOWS"}
					</div>
				)}
			</div>
							...
							{showDrops ? (
								<div className="shows-grid">
									{results.map((item, idx) => (
										<div key={item.id || idx} className="show-item">
											<div className="show-item-info">
												<Radio size={14} className="show-icon" />
												<span className="show-item-title">{formatTitle(item)}</span>
												{item.artist && (
													<span
														className="show-item-title"
														style={{
															marginLeft: "8px",
															color: "var(--text-dim)",
														}}
											...
											<div style={{ display: "flex", gap: "8px" }}>
												<button
													onClick={() => {
														const audio = new Audio(`/audio_drops/${item.mp3}`);
														audio.play();
													}}
													className="youtube-link"
													style={{ cursor: "pointer", border: "none", background: "transparent" }}
													title="Play audio drop"
												>
													<Play size={16} fill="currentColor" style={{ marginRight: "4px" }} />
													<span style={{ fontSize: "0.7rem" }}>PLAY</span>
												</button>
												<a
													href={`/audio_drops/${item.mp3}`}
													download={item.mp3}
													className="youtube-link"
													title="Download audio drop"
												>
								...
								results.map((item, idx) => (
									<div key={item.id || idx} className="result-card">
									...
									<div key={idx} className="show-item">
										...
										{show.type === "drop" ? (
											<div style={{ display: "flex", gap: "8px" }}>
												<button
													onClick={() => {
														const audio = new Audio(`/audio_drops/${show.mp3}`);
														audio.play();
													}}
													className="youtube-link"
													style={{ cursor: "pointer", border: "none", background: "transparent" }}
